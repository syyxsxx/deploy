...
void PaddleInferenceEngine::Infer(const std::vector<DataBlob> &inputs,
  ...
  for (int i = 0; i < inputs.size(); i++) {
    ...
    if (inputs[i].dtype == 0) {
      ...
      im_tensor_data = reinterpret_cast<float*>(inputs[i].data.data());
      in_tensor->copy_from_cpu(im_tensor_data);
    } else if (inputs[i].dtype == 1) {
      int64_t *im_tensor_data;
      im_tensor_data = reinterpret_cast<int64_t*>(inputs[i].data.data());
      in_tensor->copy_from_cpu(im_tensor_data);
    } else if (inputs[i].dtype == 2) {
      int *im_tensor_data;
      im_tensor_data = reinterpret_cast<int*>(inputs[i].data.data());
      in_tensor->copy_from_cpu(im_tensor_data);
    } else if (inputs[i].dtype == 3) {
      uint8_t *im_tensor_data;
      im_tensor_data = reinterpret_cast<uint8_t*>(inputs[i].data.data());
      in_tensor->copy_from_cpu(im_tensor_data);
    }
  }
  // predict
